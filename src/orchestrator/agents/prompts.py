...
2. Найди стартовую зону (если известна позиция робота — используй её, иначе — ближайшую свободную точку у края карты).
3. Определи препятствия между стартом и целью.
4. Найди узкие проходы (< safety_distance * 2) — их нужно обходить.

━━━ ТРИ СТРАТЕГИИ МАРШРУТОВ ━━━
Для каждого кандидата предложи waypoints с разной стратегией:

• "direct"   — максимально прямой путь к цели, минимум точек.
               Подходит если путь чистый. Игнорирует небольшие объезды.

• "safe"     — маршрут с запасом от стен (≥ safety_distance + 0.3м).
               Больше waypoints, обходит все препятствия с запасом.
