...
\begin{enumerate}
  ...
  \item \textbf{Orchestrator}~--- главный интеллектуальный сервис (FastAPI + OpenAI Agents SDK). Содержит маршрутизатор, планировщик, исполнитель шагов и специализированных агентов, взаимодействующих с робототехническим контуром через MCP-серверы.
...
MCP обеспечивает строгое разделение ответственности: агентный слой не зависит от конкретной реализации робототехнического контура и взаимодействует с ним исключительно через стандартизованный tool-интерфейс. Это позволяет заменять или расширять робототехнический контур без модификации кода оркестратора.
...
Маршрут команды от пользовательского запроса до робототехнического контура показан на рис.~\ref{fig:command-route}.
...
Спроектированная архитектура AgentsSwarm обеспечивает чёткое разделение ответственности между компонентами: пользовательский слой (Interface) изолирован от робототехнического контура (Mission Control/Dispatch/Isaac Sim) через двухступенчатый агентный барьер (Interface-агенты $\to$ Orchestrator-агенты $\to$ MCP-серверы).
...
Три MCP-сервера обеспечивают агентам оркестратора доступ к робототехническому контуру через стандартизованный интерфейс инструментов~\cite{mcp2024,fastmcp}. Исходные реализации MCP-серверов NVIDIA представляли прототипы с нереализованными служебными маршрутами и синхронной моделью вызовов.
...
\begin{itemize}
  ...
  \item \textit{General}~--- ответы на общие вопросы без вызова робототехнических инструментов.
\end{itemize}
...
Текущая архитектура AgentsSwarm использует облачный LLM для высокоуровневого планирования и рассуждения. Это вводит два ограничения: \textit{задержка} между запросом агента и ответом LLM (при загруженном кластере~--- от 1 до нескольких секунд); \textit{зависимость от сети}~--- при нестабильном соединении с облачным кластером робототехнический контур не может получить инструкции.
...
Следующим шагом является интеграция дистиллированной модели как кастомного VDA5050 action-handler в Mission Control. В стандарте VDA5050 поле \texttt{order} содержит последовательность узлов и рёбер, каждый из которых может включать список \texttt{actions}~--- действий, исполняемых роботом при прохождении данной точки маршрута. Регистрируя новый тип действия \texttt{vla\_action}, Mission Control перехватывает его и вместо стандартной навигационной команды ROS~2 запускает локальный инференс SmolVLA непосредственно на борту робота (целевая платформа~--- NVIDIA Jetson Orin Nano).

Типичный сценарий: оркестратор формирует навигационную миссию с action-узлом типа \texttt{vla\_action}; робот следует по маршруту до этого узла, останавливается и передаёт управление handler-у; SmolVLA получает RGB-изображение с камеры и языковую инструкцию (например, «забрать посылку»), генерирует вектор действий манипулятора и выполняет задание; после подтверждения выполнения action-а робот продолжает навигационную миссию в штатном режиме. Это добавит уровень индивидуальной бортовой автономности к централизованному контуру AgentsSwarm, обеспечив сквозное выполнение задач: от пользовательского запроса на естественном языке до физического взаимодействия робота с окружающей средой без обращения к облачному LLM-контуру.
...
\begin{enumerate}
  \item Интеграция дистиллированного SmolVLA как кастомного VDA5050 action-handler в Mission Control: оркестратор включает \texttt{vla\_action}-узлы в навигационные миссии, бортовой инференс на NVIDIA Jetson Orin Nano позволяет роботу останавливаться в заданных точках маршрута, выполнять физические задания (захват объектов и т.\,п.) и продолжать миссию~--- без обращения к облачному LLM-контуру.
...
% 6. Симуляция и робототехнические платформы (задействованы в проекте)
