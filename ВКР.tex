...
\newcommand{\placeholder}[1]{%
  \begin{center}
  \fbox{\parbox{0.85\textwidth}{\centering\small\itshape [#1]}}
  \end{center}
}
...
\textbf{Results:} eight independent components were fully implemented~--- Interface (React + FastAPI + Celery, 168~commits, v0.5.0), Orchestrator (FastAPI + Agents SDK, 56~commits, v0.5.6), vLLM Service (Data Parallel, v0.2.9), reworked Mission Control and Mission Dispatch forks, a configured ROS~2 workspace, and three heavily modified MCP servers. All 43 orchestrator API integration tests passed. The SmolVLA model was compressed 443"=fold in parameter count while retaining over 90\% accuracy (MSE~+9.1\%, $R^2$~$-$1.8\%) with a 25$\times$ inference speed"=up (450~ms~$\to$~18~ms on RTX~4090).
...
\begin{enumerate}
  ...
  \item Реализация LLM-оркестратора на базе OpenAI Agents SDK с девятью специализированными агентами.
  \item Глубокая переработка трёх MCP-серверов с переходом на асинхронную модель вызовов.
...
\textbf{FogROS2}~\cite{chen2023fogros2} демонстрирует, что offloading вычислительно тяжёлых алгоритмов (SLAM, grasp planning) в облако даёт радикальное ускорение: снижение задержки SLAM на 50\%, ускорение планирования захватов с 14~с до 1{,}2~с. \textbf{Robofleet}~\cite{sikand2021robofleet} решает задачу эффективной коммуникации в флоте с дедупликацией трафика и adaptive backpressure. Литература последовательно указывает на то, что наиболее практичны архитектуры, где низкоуровневое управление остаётся в робототехническом стеке (ROS~2, VDA5050 клиент), а облако берёт на себя планирование и семантическое reasoning~--- именно эту стратегию реализует AgentsSwarm.

\section{Большие языковые модели в робототехнике}

\subsection{Смена парадигмы: от чат-бота к reasoning-агенту}

Применение LLM в робототехнике прошло несколько этапов. Ранние работы использовали языковые модели как интерфейс «понимания команд». Переломным стало осознание, что LLM обладают commonsense reasoning и способностью к декомпозиции сложных задач. Обзоры~\cite{zeng2023llmrobotics,li2025llmmulti} классифицируют применение LLM по четырём слоям: коммуникация с человеком, перцептивная семантическая интерпретация, task planning и генерация действий. Для многоагентных систем выделяется отдельный класс задач: high-level coordination, task allocation и human-in-the-loop supervision~\cite{li2025llmmulti}.

\subsection{Парадигма ReAct и tool use}
...
\textbf{Промышленные fleet management системы} (OpenRMF, VDA5050-совместимые стеки) решают задачи координации и интероперабельности, но не интегрируют семантическое LLM-планирование. AgentsSwarm добавляет этот слой через оркестратор с девятью специализированными агентами.
...
\begin{enumerate}
  ...
  \item Генерация дополнительных артефактов: изображений, презентаций, исследовательских отчётов.
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Interface}~--- пользовательский веб-портал (React + FastAPI + Celery + RabbitMQ + Redis Streams + PostgreSQL + MinIO). Принимает запросы, маршрутизирует к агентам, стримит ответы через WebSocket.
  \item \textbf{Orchestrator}~--- главный интеллектуальный сервис (FastAPI + OpenAI Agents SDK). Содержит девять специализированных агентов, взаимодействует с роботическим контуром через MCP-серверы.
  \item \textbf{vLLM Service}~--- сервис инференса LLM в режиме Data Parallel (2\,$\times$\,Tesla V100). Предоставляет OpenAI-compatible API.
  \item \textbf{Mission Control}~--- форк NVIDIA, реализующий VDA5050-совместимый fleet manager с Behavior Tree планировщиком и cuOpt-оптимизацией.
  \item \textbf{Mission Dispatch}~--- форк NVIDIA, реализующий очередь миссий и VDA5050-обмен через MQTT.
  \item \textbf{Isaac Sim}~--- среда симуляции NVIDIA с роботами Carter в headless-режиме, интегрированная с ROS~2 Jazzy.
  \item \textbf{ROS~2 Workspace}~--- форк с дополнительными пакетами навигации, VDA5050 клиентом, rosbridge WebSocket.
  \item \textbf{SmolVLA Tools}~--- экспериментальный модуль оптимизации VLA-модели.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \texttt{ros-msp}~--- инструменты для публикации в ROS~2 топики и чтения из них через rosbridge WebSocket.
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{img/fig_2_2_mcp_servers.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{img/fig_2_3_command_route.png}
...
\textbf{Router Agent} классифицирует запросы по категориям: \textit{general} (общие вопросы), \textit{mission} (команды роботам), \textit{navigation} (задачи навигации), \textit{charging} (управление зарядкой), \textit{patrol} (патрульные маршруты), \textit{inspection} (инспекция объектов), \textit{fleet\_ops} (операции с флотом), \textit{swarm} (координация роя). Классификация выполняется через LLM с детерминированными правилами приоритетов.

\textbf{MissionPlanner} реализует двухфазный алгоритм: на первой фазе строит план в виде последовательности именованных шагов с указанием \texttt{target\_robots} и зависимостей; на второй~--- последовательно исполняет шаги, вызывая инструменты MCP-серверов.
...
\caption{Иерархия агентов оркестратора: Router Agent и специализированные агенты}
...
Параллельный канал~--- WebSocket оркестратора (\texttt{/ws/task/\{task\_id\}}), по которому SwarmOrchestratorAgent получает события от оркестратора и транслирует их в Redis Stream Interface, обеспечивая сквозную прозрачность агентного маршрута для пользователя.
...
\toprule
\textbf{Машина} & \textbf{Провайдер} & \textbf{Конфигурация} & \textbf{Назначение} \\
\midrule
...
ROS~2 Jazzy workspace собирается из форка \texttt{isaac\_ros\_common}, дополненного пакетами для отображения occupancy map, публикации costmap и rosbridge-сервером. Rosbridge WebSocket (порт 9090) обеспечивает доступ к ROS~2-топикам для MCP-сервера без необходимости установки полного стека ROS~2 на машине оркестратора.

\textbf{Occupancy map} генерируется из ортогонального вида сцены Isaac Sim, сохраняется в формате PNG и обновляется в Mission Control через endpoint \texttt{/map/update\_robot}. На основе карты Mission Control строит waypoint graph~--- ориентированный граф навигационных узлов, по которому cuOpt рассчитывает оптимальные маршруты.
...
\placeholder{Рис.~3.2. Occupancy map и waypoint graph сцены Carter Warehouse}
...
Mission Dispatch реализует очередь миссий и двустороннее VDA5050-взаимодействие между fleet manager и роботами. В оригинальном коде обнаружена проблема с legacy endpoint в пакете \texttt{packages/controllers/server}: запросы к устаревшему маршруту возвращали 404, нарушая сквозное взаимодействие.
...
\section{MCP-серверы: глубокая переработка официальных реализаций}
...
\textbf{mission-control-mcp} предоставляет инструменты: \texttt{get\_map} (PNG-карта), \texttt{get\_waypoints} (навигационные узлы), \texttt{get\_robot\_status}, \texttt{list\_robots}, \texttt{update\_map}. Реализованы заглушечные endpoints для корректной инициализации Mission Control.

\textbf{mission-dispatch-mcp} предоставляет: \texttt{create\_mission}, \texttt{cancel\_mission}, \texttt{get\_mission\_status}, \texttt{list\_missions}. Исправлены несовместимые endpoint-адреса.

\textbf{ros-msp} предоставляет: \texttt{publish\_topic}, \texttt{get\_topic\_list}, \texttt{read\_topic}. Сервер устанавливает постоянное WebSocket-соединение с rosbridge при инициализации.

\textbf{Динамический выбор транспорта} реализован через переменную окружения \texttt{MCP\_TRANSPORT}: при значении \texttt{sse} сервер запускается как HTTP-приложение; при \texttt{stdio}~--- как локальный процесс со стандартными потоками. Это позволяет использовать один код для локального тестирования и production-развёртывания.
...
\textbf{Архитектура Data Parallel:} нода 0 (координатор, rank~0) принимает HTTP-запросы, токенизирует промпт и рассылает тензоры на ноду~1 (воркер, rank~1) через gRPC-канал на порту 13345. Обе ноды независимо вычисляют forward pass по своей копии модели; координатор агрегирует логиты и генерирует следующий токен. Суммарно доступно 64~ГБ VRAM, что позволяет развернуть 32B-параметрическую модель с заполнением $\approx$~90\% памяти.

Конфигурация нод задаётся через \texttt{.env.node0} и \texttt{.env.node1}: параметры \texttt{VLLM\_HOST}, \texttt{VLLM\_PORT}, \texttt{VLLM\_GPU\_MEMORY\_UTILIZATION} (0{,}90), \texttt{TENSOR\_PARALLEL\_SIZE=1}, \texttt{PIPELINE\_PARALLEL\_SIZE=2}.
...
\textbf{Структура приложения.} FastAPI-приложение организовано по роутерам: \texttt{/health}, \texttt{/agents}, \texttt{/sessions}, \texttt{/tasks}, \texttt{/ws}. Зависимости (SessionManager, StreamCollector, MCP-клиенты) регистрируются через Depends и инициализируются при старте lifespan.

\textbf{Агентная архитектура (9 агентов).} Все агенты наследуют от базового класса \texttt{BaseAgent} и конфигурируются через JSON-файлы промптов:
\begin{itemize}
  \item \textit{Router}~--- классификация запросов по 8 категориям;
  \item \textit{MissionPlanner}~--- двухфазный алгоритм планирования и исполнения;
  \item \textit{MapAnalyst}~--- vision-LLM для пространственного анализа карты;
  \item \textit{Navigation, Charging, Patrol, Inspection}~--- специализированные агенты навигационных задач;
  \item \textit{FleetOps}~--- мониторинг и операции с флотом;
  \item \textit{SwarmCoordinator}~--- параллельная координация нескольких роботов.
\end{itemize}

\textbf{Интеграция OpenAI Agents SDK.} Вызов \texttt{Runner.run\_streamed(agent, messages, context)} возвращает асинхронный итератор событий. StreamCollector присваивает каждому событию порядковый номер (seq), буферизует и публикует в WebSocket \texttt{/ws/task/\{task\_id\}} с гарантией доставки по seq-номеру.

\textbf{SessionManager} хранит историю диалога в Redis с TTL 24~часа. Кэш результатов MCP-инструментов снижает число повторных HTTP-запросов к Mission Control.

\textbf{Guardrails} реализованы как слой валидации: входящий запрос проверяется на допустимую категорию, выходящий ответ~--- на соответствие схеме. Guardrails работают в silent fallback-режиме: невалидный ответ заменяется стандартным сообщением без прерывания сессии.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{img/fig_3_5_orchestrator_app.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.50\textwidth]{img/fig_3_6_streamcollector.png}
\caption{StreamCollector: seq-нумерация событий Agents SDK и WebSocket push-доставка}
...
Бэкенд реализован по clean architecture: слои \textit{domain} (сущности и интерфейсы), \textit{application} (ChatService, AgentExecutionService, JobOrchestrator), \textit{infrastructure} (PostgreSQL/SQLAlchemy, Redis, MinIO), \textit{api} (FastAPI роутеры, WebSocket-слой). Docker Compose предоставляет dev-стек (hot-reload) и prod-стек (Nginx + Gunicorn).
...
JobOrchestrator маршрутизирует задачу к одному из семи агентов (Табл.~\ref{tab:iface-agents}):

\begin{table}[H]
\caption{Специализированные агенты компонента Interface}
\label{tab:iface-agents}
\begin{tabular}{p{3.5cm}p{9.5cm}}
\toprule
\textbf{Агент} & \textbf{Описание} \\
\midrule
...
\begin{itemize}
  ...
  \item \textit{TracePanel}~--- timeline событий агентного маршрута: тип события, агент-источник, инструмент, временна'я метка;
...
В записанном демо показан полный сквозной сценарий: пользователь вводит запрос о навигации робота в Chat с выбором агента SwarmOrchestrator; Interface создаёт Celery-задачу, SwarmOrchestratorAgent устанавливает WebSocket-соединение с оркестратором; оркестратор выполняет Router $\to$ MissionPlanner $\to$ Navigation pipeline, вызывает MCP-инструменты, направляет робота Carter в заданную точку; результат стримится через Redis Stream и отображается в Chat и TracePanel в реальном времени.

\placeholder{Рис.~3.9. Скриншот Chat: диалог с агентом SwarmOrchestrator}

\placeholder{Рис.~3.10. Скриншот TracePanel: трассировка агентного маршрута}

\placeholder{Рис.~3.11. Скриншот Files: галерея сгенерированных артефактов}
...
\begin{itemize}
  ...
  \item \textit{Data Parallel vLLM}~--- единственная возможность уместить 32B-параметрическую модель на доступных Tesla V100;
...
\begin{table}[H]
\caption{Компоненты системы AgentsSwarm: версии и объём разработки}
\label{tab:components}
\begin{tabular}{p{3.5cm}p{2cm}p{2cm}p{5.5cm}}
\toprule
\textbf{Компонент} & \textbf{Версия} & \textbf{Коммиты} & \textbf{Стек} \\
\midrule
...
\textbf{SmolVLA}~\cite{retzlaff2025smolvla}~--- компактная VLA-модель на 450M параметров от HuggingFace/LeRobot~--- была выбрана как наиболее перспективный кандидат для бортового развёртывания благодаря открытым весам, документированному API LeRobot и наличию асинхронного inference stack.
...
\textbf{Этап~1: Knowledge Distillation (KD).} Student-модель строится как уменьшенная копия Teacher с коэффициентом \texttt{student\_ratio} (отношение числа attention heads и скрытых размерностей к базовым). Функция потерь:
\begin{equation}
\mathcal{L}_{KD} = \alpha \cdot \mathcal{L}_{MSE} + (1-\alpha) \cdot \mathcal{L}_{KL} + \lambda \cdot \mathcal{L}_{AT},
\label{eq:kd-loss}
\end{equation}
...
\textbf{Этап~2: Mixed Precision Inference (FP16).} Применяется \texttt{torch.cuda.amp} с автоматическим управлением precision: веса остаются в FP32, активации вычисляются в FP16. Ускорение инференса в 1{,}5--2~раза без дополнительной потери точности.

\textbf{Этап~3: Structured Pruning.} Обнуляется 30\% весов в Linear-слоях по критерию наименьшей $L_2$-нормы. Прунинг применяется к слоям attention projection и feed-forward, нетронутыми остаются слои нормализации и embedding.

\textbf{Этап~4: INT8 Quantization Analysis.} Оценивается применимость INT8-квантизации к каждому слою по метрике чувствительности. Выявлены критические слои (первый и последний Linear в каждом transformer-блоке), для которых квантизация приводит к значительной деградации точности; они исключаются из схемы.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{img/fig_4_1_smolvla_pipeline.png}
...
Стратегия верификации AgentsSwarm охватывает три уровня: \textit{unit-тесты} базовых сервисов и вспомогательных функций; \textit{контрактные тесты} поведения ключевых агентов; \textit{интеграционные тесты} HTTP API. Все тесты реализованы на pytest с использованием Docker Compose для запуска необходимой инфраструктуры (Redis, RabbitMQ, PostgreSQL). Покрытые компоненты: Orchestrator и Interface~--- два наиболее значимых сервиса системы.

\section{Тестирование оркестратора}

Тестовый набор оркестратора включает 43 теста, организованных по четырём группам.

\textbf{Unit-тесты базовых сервисов} проверяют: корректность health-check эндпоинта, инициализацию SessionManager в Redis, seq-нумерацию StreamCollector при пакетной публикации, поведение retry-механизма при имитации сетевых ошибок.

\textbf{Контрактные тесты планировщика} верифицируют структуру выходных данных MissionPlanner: обязательное наличие полей \texttt{steps}, \texttt{target\_robots} и \texttt{dependencies}; корректность ссылок зависимостей; непустоту \texttt{target\_robots} для mission-категорий.

\textbf{Тесты guardrails} проверяют граничные сценарии: пустой запрос возвращает standard error-ответ; запрос вне компетенции агента~--- silent fallback; попытка handoff к несуществующему агенту~--- перехват на уровне guardrail.

\textbf{Интеграционные тесты HTTP API} проверяют все эндпоинты с реальным FastAPI-приложением. Результат: \textbf{43/43 тестов пройдено}~$\checkmark$. Покрытые сценарии: создание сессии, отправка сообщения, получение статуса задачи, история диалога, принудительная отмена, graceful shutdown.

\section{Тестирование Interface}

Pytest-покрытие Interface включает: эндпоинты управления чатами (создание, получение, удаление), загрузку файлов с верификацией сохранения в MinIO, маршрутизацию к агентам с mock-объектами Celery-задач, авторизацию WebSocket-соединений.
...
\textbf{Smoke-тест полного стека} запускает docker-compose с минимальной конфигурацией (FastAPI + Redis + RabbitMQ + Celery) и выполняет сценарий: POST \texttt{/api/chats/} $\to$ POST \texttt{/api/chats/\{id\}/messages/} $\to$ WebSocket \texttt{/ws/chat/\{id\}} $\to$ ожидание события \texttt{agent\_reply}. Тест подтверждает корректную работу всей цепочки без реального LLM-бэкенда.
...
Для VLA-модели SmolVLA достигнуто сжатие в 443 раза по числу параметров при сохранении более 90\% точности (MSE~+9{,}1\%, $R^2$~$-$1{,}8\%) и ускорении инференса в 25~раз (450~мс~$\to$~18~мс, RTX~4090).
...
\addcontentsline{toc}{section}{Приложение А. Глоссарий предметной области}
...
\addcontentsline{toc}{section}{Приложение В. Технические характеристики инфраструктуры}
...
\addcontentsline{toc}{section}{Приложение Г. Ключевые фрагменты кода}
...
async def main():
    ...
    if transport == "sse":
        ...
        async with SseServerTransport(port) as sse:
            await server.run(sse.read_stream, sse.write_stream)
    else:
...
class StreamCollector:
    def __init__(self, task_id: str, redis: Redis):
    ...
    async def publish(self, event_type: str, data: dict):
        self._seq += 1
        payload = {"seq": self._seq, "type": event_type, **data}
...
def distillation_loss(student_out, teacher_out, student_attn,
                      teacher_attn, alpha=0.7, T=3.0, lam=0.1):
    ...
    at = sum(F.mse_loss(s, t) for s, t in zip(student_attn, teacher_attn))
    return alpha * mse + (1 - alpha) * kl + lam * at
\end{lstlisting}

\chapter{Протоколы экспериментов}
\addcontentsline{toc}{section}{Приложение Д. Протоколы экспериментов}

\subsection*{Д.1. Протокол тестирования API оркестратора}

Среда: Ubuntu 24.04, Python 3.11, pytest 8.x, Redis 7.x (Docker). Конфигурация тестов: переменные окружения \texttt{TEST\_MODE=true}, \texttt{MOCK\_LLM=true} (ответы агентов заменяются mock-объектами). Команда запуска: \texttt{pytest tests/ -v --tb=short}. Результат: 43 пройдено, 0 провалено, 0 пропущено. Время выполнения: $\approx$\,45~с.

\subsection*{Д.2. Протокол эксперимента по дистилляции SmolVLA}

Среда: Ubuntu 22.04, CUDA 12.1, PyTorch 2.2, RTX~4090 (24~ГБ VRAM). Датасет: \texttt{lerobot/libero} (train: 18\,000 траекторий, val: 2\,000 траекторий). Гиперпараметры: lr~=~1e-4, batch\_size~=~32, epochs~=~10, $T$~=~3{,}0, $\alpha$~=~0{,}7, $\lambda$~=~0{,}1, student\_ratio~=~0{,}1. Время обучения: $\approx$\,4~ч.
