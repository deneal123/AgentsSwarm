...
### 3.7. Interface: веб-платформа управления и пользовательского взаимодействия `[~8–10 с.]`

#### 3.7.1. Общая архитектура и стек

- Декомпозиция: React-фронтенд + FastAPI-бэкенд + Celery-воркеры + RabbitMQ + Redis Streams + MinIO + PostgreSQL (Рис. 3.7)
- Clean architecture бэкенда: слои domain / application / infrastructure / api
- Docker Compose стеки: dev (hot-reload) и prod (Nginx + Gunicorn)
- Версионирование: v0.5.0, 168 коммитов

#### 3.7.2. Backend: агенты и обработка задач

- Семь специализированных агентов: General, WebSearch, DeepResearch, ImageGen, PptxGen, AudioTranscribe, SwarmOrchestrator (Табл. 3.6)
- Реализация агентного роутинга: классификация запроса, выбор агента, JobOrchestrator
- Celery-задачи: `process_agent_message`, очереди RabbitMQ, retry-политики
- SwarmOrchestratorAgent: проксирование запросов в Orchestrator через WebSocket/REST
- Хранение истории чатов: PostgreSQL + SQLAlchemy, Alembic-миграции
- Файловое хранилище: MinIO для генерируемых артефактов (изображения, PPTX, аудио)

#### 3.7.3. Потоковая модель: Redis Streams и WebSocket

- Redis Stream `chat:{thread_id}:stream`: запись агентных событий через `XADD` (Рис. 3.8)
- WebSocket-слой: авторизация → XREAD → heartbeat → push клиенту
- Типы событий: `stream_chunk`, `agent_reply`, `routing_event`, `tool_event`
- Жизненный цикл ответа: pending → streaming → completed / error

#### 3.7.4. Frontend: интерфейс пользователя

- Стек: React + Chakra UI; страницы: Chat, TracePanel, Files, Profile / Memory (Рис. 3.9)
- Chat-страница: поле ввода, история сообщений, индикатор печати, файловые вложения
- TracePanel: визуализация событий агентного маршрута в реальном времени
- Files: список сгенерированных файлов с предпросмотром и скачиванием
- Profile / Memory: персональная память пользователя (интеграция Mem0)
- Адаптивный дизайн: тёмная тема, skeleton-загрузка, error boundaries

#### 3.7.5. Демонстрация и визуализация рабочего процесса

- Описание записанного demo-сценария: полный воркфлоу от запроса до ответа SwarmOrchestrator (Рис. 3.10)
- Экранные снимки ключевых состояний:
  - Диалог с General-агентом (Рис. 3.11)
  - Запрос web/deep-research с итоговым отчётом (Рис. 3.12)
  - Генерация изображения / PPTX и просмотр в Files (Рис. 3.13)
  - TracePanel с трассировкой маршрута агента (Рис. 3.14)
  - Управление роем через SwarmOrchestrator в чате (Рис. 3.15)
- Анализ UX-сценариев: время от запроса до первого чанка, задержка стриминга

### 3.8. Выводы и результаты по главе 3
- Реализованные компоненты: сводная таблица версий и объёма (Табл. 3.7)
- Ключевые технические решения и их обоснование
- 43/43 интеграционных теста API оркестратора

---

## Глава 4. Оптимизация модели SmolVLA для бортового инференса `[~8–10 с.]`
...
## Глава 5. Верификация системы `[~5 с.]`

### 5.1. Стратегия тестирования

- Уровни тестирования: unit, контрактные, интеграционные
- Инструменты: pytest, Docker-окружение; покрытые компоненты: Orchestrator, Interface
...
- Pytest-покрытие: эндпоинты чатов, загрузка файлов, агентный роутинг
- Проверка стриминга: корректность XADD / XREAD, доставка через WebSocket
- Smoke-тест полного стека: поднятие docker-compose + запуск сценария через чат

### 5.4. Выводы и результаты по главе 5

- Количественные результаты автоматизированного тестирования
- Выявленные ограничения: LLM-слой (risk of hallucination), отсутствие формальной верификации планов
