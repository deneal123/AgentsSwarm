"""
MinIO/S3 storage backend with presigned URLs and retry logic
"""
...
class MinioFileStorage(AbstractFileStorage):
    ...
    Features:
    - File upload/download/delete with retry logic
    - Presigned URLs for secure direct access (download & upload)
    - Automatic bucket creation
    - Exponential backoff retry strategy
    - Public endpoint rewriting for browser access
    """
    ...
    def __init__(self, config: MinioConfig) -> None:
        ...
        Args:
            config: MinioConfig with connection details

        Raises:
            RuntimeError: If minio client not installed or bucket creation fails
        """
        try:
            from minio import Minio  # type: ignore
        except ImportError as e:
            raise RuntimeError("Minio client not installed. Install with: pip install minio") from e
    ...
    def file_exists(self, file_key: str) -> bool:
        ...
        Args:
            file_key: Storage key (object name)
    ...
    async def get_file(self, file_key: str) -> bytes:
        ...
        try:
            ...
            raise FileNotFoundError(f"File not found or download failed: {file_key}") from e
