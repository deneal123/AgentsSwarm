...
The Isaac ROS environment is managed and accessed via the Isaac ROS CLI in one of three isolation modes:
...
By default, the Isaac ROS CLI uses the image key sequence [isaac_ros], and NVIDIA provides a corresponding prebuilt Docker image via the NGC Container Registry.
...
Since NVIDIA only provides prebuilt Docker images for the base image key sequence [isaac_ros], any additional image keys must be built locally.
