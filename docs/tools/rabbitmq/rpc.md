...
In order to receive a response we need to send a 'callback' queue name with the request. Such a queue is often server-named but can also have a well-known name (be client-named).
