...
def upgrade() -> None:
    ...
    op.create_index("ix_profile_billing_webhooks_webhook_id", "billing_webhooks", ["webhook_id"], unique=True, schema="profile")
    op.create_index("ix_profile_billing_webhooks_created_at", "billing_webhooks", ["created_at"], schema="profile")


def downgrade() -> None:
    op.drop_index("ix_profile_billing_webhooks_created_at", table_name="billing_webhooks", schema="profile")
    op.drop_index("ix_profile_billing_webhooks_webhook_id", table_name="billing_webhooks", schema="profile")
