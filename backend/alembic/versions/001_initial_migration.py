...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('is_payment_taken', sa.Boolean(), nullable=False, server_default=sa.text('false')),
    ...
    op.create_table(
        ...
        sa.Column('thread_id', sa.BigInteger(), sa.ForeignKey('profile.chat_threads.id', ondelete='CASCADE')),
        sa.Column('user_id', postgresql.UUID(), nullable=True),
    ...
    op.create_index('ix_profile_chat_threads_user_id', 'chat_threads', ['user_id'], schema='profile')
    op.create_index('ix_profile_chat_messages_thread_id', 'chat_messages', ['thread_id'], schema='profile')
    ...
    op.create_index('ix_profile_nutrition_calendars_user_id', 'nutrition_calendars', ['user_id'], schema='profile')
    ...
    op.create_index('ix_profile_batch_items_batch_id', 'batch_items', ['batch_id'], schema='profile')

    # Billing / Token quotas
    op.create_table(
        'token_quotas',
        sa.Column('id', sa.BigInteger(), primary_key=True),
        sa.Column('user_id', postgresql.UUID(), sa.ForeignKey('profile.user.id', ondelete='CASCADE')),
    ...
    op.create_table(
        'billing_events',
        sa.Column('id', sa.BigInteger(), primary_key=True),
        sa.Column('user_id', postgresql.UUID(), sa.ForeignKey('profile.user.id', ondelete='SET NULL')),
...
def downgrade() -> None:
    ...
    op.drop_index('ix_profile_billing_events_user_id', table_name='billing_events', schema='profile')
    op.drop_index('ix_profile_token_reservations_reservation_id', table_name='token_reservations', schema='profile')
    ...
    op.drop_index('ix_profile_nutrition_calendars_user_id', table_name='nutrition_calendars', schema='profile')
    op.drop_index('ix_profile_chat_messages_thread_id', table_name='chat_messages', schema='profile')
