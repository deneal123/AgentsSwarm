...
from service.repositories.exceptions import RepositoryError
...
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    logger.warning("HTTP Exception occurred: %s - %s for request: %s %s", exc.status_code, exc.detail, request.method, request.url)
    payload = map_exception_to_error_response(exc)
    return JSONResponse(status_code=exc.status_code, content=payload.model_dump())


async def validation_exception_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
    logger.warning("Validation error occurred for request: %s %s - %s", request.method, request.url, exc.errors())
    errors = []
    ...
    exception = HTTPException(
        ...
        detail={"code": "validation_error", "message": "Validation error", "type": "ValidationError", "details": errors},
    )
...
async def repository_exception_handler(request: Request, exc: RepositoryError) -> JSONResponse:
    status_code = map_exception_to_status(exc)
    logger.warning("%s occurred for request %s %s: %s", exc.__class__.__name__, request.method, request.url, exc)
    payload = map_exception_to_error_response(exc)
    return JSONResponse(status_code=status_code, content=payload.model_dump())
