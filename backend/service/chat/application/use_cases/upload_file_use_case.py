...
class UploadFileUseCase:
    ...
    async def execute(self, *, filename: str, content_type: str | None, content_bytes: bytes, thread_id: str, user_id: str | None) -> dict:
        ...
        saved_file_id, saved_file_url, saved_file_key = await self._save_file(filename, content_bytes, user_id)
    ...
    async def _extract_text(self, filename: str, content_type: str | None, content_bytes: bytes) -> tuple[str, str]:
        ...
        if lower_name.endswith((".png", ".jpg", ".jpeg", ".gif", ".webp")):
            return await self.media_analysis_port.analyze_image(content_bytes, content_type or "image/png", filename), "image"
