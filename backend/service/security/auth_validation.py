...
class AuthValidator:
    ...
    def validate_http_jwt(self, auth_token: str | None) -> AuthProfile:
        ...
        try:
            ...
            self._telemetry.record_failure(surface="http", reason="jwt_expired", channel="jwt_cookie")
            raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail="Token expired")
        except jwt.InvalidTokenError:
            self._telemetry.record_failure(surface="http", reason="jwt_invalid", channel="jwt_cookie")
    ...
    async def authenticate_websocket(self, websocket: WebSocket, session_store) -> dict | None:
        ...
        if jwt_cookie:
            ...
            try:
                ...
                self._telemetry.record_failure(surface="ws", reason="jwt_invalid", channel="jwt_cookie")
                return None
        ...
        if token == "anon-token":
            if not self._is_channel_allowed("anon_token"):
                self._telemetry.record_failure(surface="ws", reason="channel_blocked", channel="anon_token")
                return None
            return {"id": "anon-session", "user_id": "00000000-0000-0000-0000-000000000000"}

        if token == "test-token":
            if self._auth_config.auth_mode == "prod" and self._auth_config.enforce_prod_runtime_auth_guard:
                self._telemetry.record_failure(
                    surface="ws", reason="dev_token_forbidden_prod", channel=token_candidate.channel
                )
                return None
            if not self._auth_config.enable_dev_test_token:
                self._telemetry.record_failure(
                    surface="ws", reason="dev_token_feature_disabled", channel=token_candidate.channel
                )
                return None
    ...
    def _get_ws_token_candidate(self, websocket: WebSocket) -> WsTokenCandidate | None:
        ...
        if header.lower().startswith("bearer "):
            return WsTokenCandidate(channel="authorization_bearer", token=header.split(" ", 1)[1].strip())
