...
class ChatAgent:
    ...
    def __init__(self, config: Optional[dict] = None, stream_port: StreamPort | None = None):
    ...
    async def handle_message(
        ...
        try:
            ...
            try:
                async for event in self.processor.process_message_stream(
                    ...
                    try:
                        ...
                        logger.error(f"CRITICAL ERROR: Failed to publish {event.type} event to Redis: {e}", exc_info=True)
                ...
                logger.error(f"FATAL ERROR: Event processing loop failed: {loop_error}", exc_info=True)
                logger.error(f"Last processed event count: {events_processed}")
            logger.info(f"LOOP ENDED: Finished processing {events_processed} events, has_streaming: {has_streaming}, collected_chunks: {len(collected_chunks)}")
            ...
            if not has_streaming and reply_text:
                logger.info(f"No streaming chunks generated, simulating streaming for reply of length {len(reply_text)}")
                ...
                for i in range(0, len(reply_text), chunk_size):
                    ...
                    stream_event = {
                        "type": "stream_chunk",
                        "data": chunk,
                        "job_id": f"job_{thread_id}_{int(datetime.now(timezone.utc).timestamp())}",
                        "seq": i // chunk_size + 1,
