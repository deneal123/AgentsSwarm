"""Tooling package for agent-executable capabilities."""

from service.agents.tools.router import route_model
from service.agents.tools.web_search import parse_url, web_search, web_search_and_summarize
from service.agents.tools.deep_research import deep_research
from service.agents.tools.pptx import generate_pptx
from service.agents.tools.function_tools import DEFAULT_FUNCTION_TOOLS

__all__ = [
    "route_model",
    "web_search",
    "parse_url",
    "web_search_and_summarize",
    "deep_research",
    "generate_pptx",
    "DEFAULT_FUNCTION_TOOLS",
]
