...
from service.agents import client as agents_client  # noqa: F401
from service.agents.client import create_chat_completion, list_available_models, get_active_provider
...
class SimpleStreamingAgent(BaseAgent):
    ...
    async def process(
        ...
        try:
            ...
            # Build SDK agent
            # Convert model_settings dict to ModelSettings if needed
            ...
            # Some providers/SDK paths may not emit deltas, but final output can still exist.
            ...
            # Fallback path for providers that reject Responses API (e.g. 403 on streamed run).
            try:
                selected_model = None
                if isinstance(self.model_settings, dict):
...
class CollectorGeneratorAgent(BaseAgent):
    ...
    async def _collector_phase(
        ...
        try:
            # Build collector agent with tools
    ...
    async def _generator_phase(
        ...
        try:
            # Build generator agent WITHOUT tools
            ...
            generator = SDKAgent(
                ...
                tools=[],  # NO TOOLS in generator phase
                model_settings=ms
            )
            
            # Enhanced context with collected data
            try:
                ctx_payload = context.model_dump() if hasattr(context, "model_dump") else context.dict()
            except Exception:
                ctx_payload = {"user_id": str(context.user_id)}
