...
class JobRepository(BaseRepository):
    ...
    async def fetch_stale_processing_jobs(
        ...
        """
        Fetch PROCESSING jobs that are older than stale_threshold_sec.
        These jobs may be stuck and need cleanup.
        """
        assert session is not None, "DB session is required"
    ...
    async def delete_old_completed_jobs(
        ...
        """
        Delete old completed/failed jobs older than retention_days.
        ...
        # First, select the IDs to delete (for logging/limiting)
        stmt = (
        ...
        delete_stmt = delete(UserLaunch).where(UserLaunch.id.in_(job_ids))
