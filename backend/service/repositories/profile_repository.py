...
class ProfileRepository(BaseRepository):
    ...
    async def create_user(
        ...
        new_user = User(
            ...
            available_launches=10,  # Default value for new users
        )
    ...
    async def delete_user_chat_history(
        ...
        assert session is not None, "DB session is required"
        """Delete (hard) all chat threads and messages for a user.

        For auditability an alternative is to do a soft-delete by adding
        'deleted' flags; here we perform cascade delete.
        """
