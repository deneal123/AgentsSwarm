...
class DefaultAgentExecutionService(AgentExecutionPort):
    async def execute(
        ...
        *,
        ...
        resolved_model, routing_meta = await RouteModelUseCase().execute(text=text, selected_model=selected_model, input_type=input_type)
        ...
        reply, metadata = PostprocessAgentReplyUseCase().execute(reply_assembler=reply_assembler, metadata=metadata)
