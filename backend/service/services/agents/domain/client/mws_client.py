...
try:
    ...
    # Keep client usable even if OpenAI Agents SDK is absent in minimal envs.
...
DEFAULT_MWS_BASE_URL = "https://api.gpt.mws.ru/v1"

_MODELS_CACHE: dict[str, object] = {"items": [], "expires_at": 0.0}
_MODELS_LOCK = asyncio.Lock()


def _resolve_mws_api_key() -> str:
    # Primary source for the hackathon integration.
    if config.agents.mws_api_key:
        return config.agents.mws_api_key
    # Backward compatibility with existing env naming.
    return config.agents.openai_api_key


def _resolve_mws_base_url() -> str:
    raw = (config.agents.mws_base_url or config.agents.openai_base_url or "").strip()
    if not raw:
        raw = DEFAULT_MWS_BASE_URL
    base = raw.rstrip("/")
    # MWS docs use /v1 endpoints; normalize if user provided host without suffix.
...
def _make_http_client() -> httpx.AsyncClient:
    timeout = config.agents.mws_timeout_sec or DEFAULT_MWS_TIMEOUT_SEC
    ...
    try:
        ...
        logger.exception("Failed to build HTTP client for MWS GPT")
        return httpx.AsyncClient(timeout=timeout)
...
def create_mws_client() -> AsyncOpenAI | None:
    ...
    try:
        ...
        logger.exception("Failed to initialize MWS GPT client")
        return None
...
def _models_cache_ttl() -> int:
    ttl = config.agents.mws_models_cache_ttl_sec or DEFAULT_MODELS_CACHE_TTL_SEC
...
async def list_available_models(
    ...
    async with _MODELS_LOCK:
        ...
        try:
            ...
            logger.exception("Failed to fetch available models from MWS GPT API")
...
async def create_chat_completion(
    ...
    *,
    ...
    target_client = client or OPENAI_CLIENT
    if target_client is None:
        raise RuntimeError("MWS GPT client is not configured")
