...
from service.services.agents.client import get_openai_client, list_available_models
...
class OpenAIMediaAnalysisAdapter(MediaAnalysisPort):
    async def analyze_image(self, content_bytes: bytes, content_type: str, filename: str) -> str:
        ...
        image_data_url = f"data:{content_type or 'image/png'};base64,{base64.b64encode(content_bytes).decode()}"
        models = await list_available_models()
        vlm_re = re.compile(r"(vision|vl\b|vlm|multimodal|image|qwen.*vl|llava|gpt-4o|pixtral)", re.I)
        ...
        resp = await client.chat.completions.create(
            ...
            messages=[{
                ...
                "content": [
                    ...
                    {"type": "text", "text": "Опиши подробно что изображено на этой картинке. Укажи все важные детали, объекты, текст, данные — всё что видишь."},
                ],
            }],
            max_tokens=1000,
        )
        return getattr(resp.choices[0].message, "content", "") or f"[Изображение загружено: {filename}]"
    ...
    async def transcribe_audio(self, content_bytes: bytes, filename: str) -> str:
        ...
        for model_id in candidates:
            ...
            try:
                transcription = await client.audio.transcriptions.create(model=model_id, file=audio_file)
