...
class ChatPersistenceService:
    ...
    async def persist_messages(
        self, thread_id: str, user_text: str, agent_reply: str, user_id: str | int | None
    ) -> None:
        thread_pk = await self.repo.get_thread_pk(thread_id)
        if thread_pk is None:
            return
        await self.repo.insert_message(
            thread_pk=thread_pk, sender="user", content=user_text, user_id=user_id
        )
        await self.repo.insert_message(
            thread_pk=thread_pk, sender="agent", content=agent_reply, user_id=None
        )
