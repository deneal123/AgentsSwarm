...
from service.agents.tools.router import (
    _parse_llm_response,
    _pick_router_model,
    route_model,
)
...
def test_route_model_uses_llm_router_result():
    ...
    with patch("service.agents.tools.router._llm_route", new=AsyncMock(return_value=llm_result)):
        model, meta = asyncio.run(route_model(
...
def test_route_model_llm_receives_input_type():
    ...
    with patch("service.agents.tools.router._llm_route", new=AsyncMock(return_value=llm_result)) as mock:
        model, meta = asyncio.run(route_model(
...
def test_route_model_regex_fallback_on_llm_failure():
    with patch("service.agents.tools.router._llm_route", new=AsyncMock(return_value=None)):
        model, meta = asyncio.run(route_model(
            text="```python\nimport os\nprint('hi')\n```\nfix bug",
            selected_model=None,
            input_type="text",
